...
class Cart:
    ...
    def add_item(self, item: dict, qty: int = 1) -> None:
        # TODO: validate FIRST, then mutate.
          if qty < 1:                 raise ValueError("Quantity must be a positive integer.")
          if not item["available"]:   raise OutOfStockError("Item is not available.")
        # raise NotImplementedError

    def remove_item(self, item_id: int) -> None:
        # TODO: raise KeyError if the item is not in the cart
        if not any(line["id"] == item_id for line in self.lines):
            raise KeyError("Item is not in the cart.")
        # raise NotImplementedError
